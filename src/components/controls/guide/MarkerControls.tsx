...
import { GuideMarker } from '../../../types/guide/common';

interface MarkerControlsProps {
  selectedExample?: string;
  selectedMarker?: number | null;
  markerPosition?: { lat: number; lng: number };
  interactiveMarkers?: GuideMarker[];
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
          {interactiveMarkers.map(marker => (
            <div
              ...
              style={{
                padding: '10px',
                backgroundColor: selectedMarker === Number(marker.id) ? '#fff3cd' : 'white',
                border: `2px solid ${selectedMarker === Number(marker.id) ? '#ffc107' : '#e0e0e0'}`,
                borderRadius: '8px',
                cursor: 'pointer',
                transition: 'all 0.3s ease'
              }}
              onClick={() => onMarkerClick?.(Number(marker.id))}
            >
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '5px' }}>
                <div
                  style={{
                    width: '12px',
                    height: '12px',
                    backgroundColor: selectedMarker === Number(marker.id) ? '#FFD700' : '#007bff',
                    borderRadius: '50%',
                    marginRight: '8px'
                  }}
                />
        ...
        {selectedMarker && (
          <div style={{
            ...
            <strong>Selected:</strong> {interactiveMarkers.find(m => Number(m.id) === selectedMarker)?.title}
          </div>
        )}
      </div>
    );
